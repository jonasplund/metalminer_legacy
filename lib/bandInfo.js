(function () {
    ...
    bi.wikipedia = function (metaInfo, callback) {
        ...
        var uri = 'http://www.google.com/search?q=' + metaInfo.artist.replace(/ /g, '+') + 
            '+band+site:en.wikipedia.org';
        uri = encodeURI(uri);
        request({ uri: uri }, function (err, res, body) {
            ...
            linkhref = linkhref.replace('en.wikipedia.org', 'en.mobile.wikipedia.org').
                replace('/url?q=', '').replace(/\&sa=.*/, '');
            request({ uri: linkhref }, function (err, res, body) {
                ...
                $('.metadata').remove();
                var html = $('#content').html().replace(/Jump back a section/g, '').replace(/Edit/g, '').trim();
    ...
    bi.metalArchives = function (metaInfo, callback) {
        ...
        uri += metaInfo.artist.replace(/ /gi, '+') + '&releaseTitle=' + 
            metaInfo.album.replace(/\(.*?\)/gi, '').replace(/ /gi, '+');
